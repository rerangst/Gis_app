...
namespace App_tmp
{
    class DataProvider
    {
        ...
        NpgsqlConnection conn = new NpgsqlConnection("Server=127.0.0.1;Port=5432;User Id=postgres;Password=56tyghbn;Database=tmpData;");
        ...
        public byte[] GetImage(string tbName, string rowName)
        {
            this.OpenConn();
            string query = "select image from "+tbName+" WHERE ten = @rowName";
            var cmd = new NpgsqlCommand(query, conn);
            //cmd.Parameters.Add("table", tbName);
            cmd.Parameters.Add("rowName", rowName);
            var reader = cmd.ExecuteReader();
            reader.Read();
            byte[] xByte = (byte[])reader[0];
            CloseConn();
            return xByte;
        }
